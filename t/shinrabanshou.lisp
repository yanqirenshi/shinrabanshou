...
(let ((vertex-note-1 "n-note-1")
  ...
  (let* ((vertex1 (tx-make-vertex pool 'test-vertex `((note ,vertex-note-1))))
    ...
    (is 2 (length (find-object-with-slot pool 'edge 'shinra::from (get-id vertex1))))
    ...
    (let ((tmp-edge-1 (find-object-with-id pool 'edge (get-id edge1)))
      ...
      (is (get-id vertex1) (get-from-vertex-id tmp-edge-1))
      (is (get-id vertex2) (get-to-vertex-id   tmp-edge-1))
      (is (get-id vertex1) (get-from-vertex-id tmp-edge-2))
      (is (get-id vertex3) (get-to-vertex-id   tmp-edge-2))
      ;; check class. from and to vertex.
      (is (class-name (class-of vertex1))
          (get-from-vertex-class tmp-edge-1))
      (is (class-name (class-of vertex2))
          (get-to-vertex-class tmp-edge-1))
      (is (class-name (class-of vertex1))
          (get-from-vertex-class tmp-edge-2))
      (is (class-name (class-of vertex3))
          (get-to-vertex-class tmp-edge-2))
      (is vertex1 (find-object-with-id pool (get-from-vertex-class tmp-edge-1) (get-id vertex1)))
      (is vertex2 (find-object-with-id pool (get-to-vertex-class tmp-edge-1)   (get-id vertex2)))
      (is vertex1 (find-object-with-id pool (get-from-vertex-class tmp-edge-2) (get-id vertex1)))
      (is vertex3 (find-object-with-id pool (get-to-vertex-class tmp-edge-2)   (get-id vertex3)))
      )
       ;;;
       ;;;
    (up:snapshot pool)
  ...
  (let ((vertex-note-1 "n-note-1")
    ...
    (let* ((vertex1 (tx-make-vertex pool rsc-class `((note ,vertex-note-1))))
      ...
      (let ((i-from (get-root-object pool (up::get-objects-slot-index-name edge-class 'from)))
        ...
        ;; from
        (is 2 (hash-table-count (gethash (get-from-vertex-id edge1) i-from)))
        (up:tx-remove-object-on-slot-index pool edge1 'shinra::from)
        (is 1 (hash-table-count (gethash (get-from-vertex-id edge1) i-from)))
        (is 1 (hash-table-count i-from))
        (is 2 (hash-table-count i-to))
        (is 1 (hash-table-count i-type))
        ;; to
        (is 1 (hash-table-count (gethash (get-to-vertex-id edge1) i-to)))
        (up:tx-remove-object-on-slot-index pool edge1 'shinra::to)
        (is 0 (hash-table-count (gethash (get-to-vertex-id edge1) i-to)))
        (is 1 (hash-table-count i-from))
        (is 2 (hash-table-count i-to))
        (is 1 (hash-table-count i-type))
        ;; type
...
(let ((vertex-note-1 "n-note-1")
  ...
  (let* ((vertex1 (tx-make-vertex pool rsc-class `((note ,vertex-note-1))))
    ...
    (let ((i-from (get-root-object pool (up::get-objects-slot-index-name edge-class 'from)))
      ...
      (is 2 (length (find-object-with-slot pool edge-class 'shinra::from (get-from-vertex-id edge1))))
      (is 1 (length
             (find-object-with-slot pool edge-class 'shinra::to (get-to-vertex-id edge1))))
      ...
      (is 1 (length (find-object-with-slot pool edge-class 'shinra::from (get-from-vertex-id edge1))))
      (is 0 (length
             (find-object-with-slot pool edge-class 'shinra::to (get-to-vertex-id edge1))))
...
(let ((vertex-note-1 "n-note-1")
  ...
  (let* ((vertex1 (tx-make-vertex pool 'test-vertex `((note ,vertex-note-1))))
         ...
         (vertex3 (tx-make-vertex pool 'test-vertex `((note ,vertex-note-3))))
         (edge1 (tx-make-edge pool 'edge vertex1 vertex2 edge-type)))
    (is (get-id vertex1) (get-from-vertex-id edge1))
    (is (get-id vertex2) (get-to-vertex-id edge1))
    (is edge1 (tx-change-vertex pool edge1 :from vertex3))
    (is (get-id vertex3) (get-from-vertex-id edge1))
    (is (get-id vertex2) (get-to-vertex-id edge1))
    (is edge1 (tx-change-vertex pool edge1 :to vertex1))
    (is (get-id vertex3) (get-from-vertex-id edge1))
    (is (get-id vertex1) (get-to-vertex-id edge1))
    (up:snapshot pool)
    ))
