...
(test test@basic
  (let ((node-note-1 "n-note-1")
    ...
    (let* ((node1 (tx-make-node pool 'test-node `((note ,node-note-1))))
      ...
      ;;; check node
      (is-true (eq node1 (find-object-with-id pool 'test-node (get-id node1))))
      (is-true (eq node2 (find-object-with-id pool 'test-node (get-id node2))))
      (is-true (find-object-with-slot pool 'test-node 'note node-note-1))
      (is-true (find-object-with-slot pool 'test-node 'note node-note-2))
      ;;; check edge
