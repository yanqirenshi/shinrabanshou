...
(let ((vertex-note-1 "n-note-1")
  ...
  (let* ((vertex1 (tx-make-vertex pool 'vertex `((note ,vertex-note-1))))
         ...
         (edge1 (tx-make-edge pool 'ra vertex1 vertex2 edge-type))
         (edge2 (tx-make-edge pool 'ra vertex1 vertex3 edge-type)))
       ;;; check vertex
    (is vertex1 (find-object-with-id pool 'vertex (id vertex1)))
    (is vertex2 (find-object-with-id pool 'vertex (id vertex2)))
    (ok (find-object-with-slot pool 'vertex 'note vertex-note-1))
    (ok (find-object-with-slot pool 'vertex 'note vertex-note-2))
       ;;; check edge
    (is 2 (length (get-root-object pool :RA-ROOT)))
    (is 2 (length (find-object-with-slot pool 'ra 'shinra::edge-type :test-r)))
    (is 2 (length (find-object-with-slot pool 'ra 'shinra::from-id (id vertex1))))
    (is edge1 (find-object-with-id pool 'ra (id edge1)))
    (is edge2 (find-object-with-id pool 'ra (id edge2)))
    ;;
    (let ((tmp-edge-1 (find-object-with-id pool 'ra (id edge1)))
          (tmp-edge-2 (find-object-with-id pool 'ra (id edge2))))
...
(let ((vertex-note-1 "n-note-1")
  ...
  (let* ((vertex1 (tx-make-vertex pool 'vertex `((note ,vertex-note-1))))
         ...
         (edge2 (tx-make-edge pool 'ra vertex1 vertex3 edge-type)))
    ;; find-r
    (is 2 (length (find-r-edge pool 'ra :from vertex1)))
    (is 0 (length (find-r-edge pool 'ra :to   vertex1)))
    (is 1 (length (find-r-edge pool 'ra :to   vertex2)))
    (is 1 (length (find-r-edge pool 'ra :to   vertex3)))
    (is 0 (length (find-r-edge pool 'ra :from vertex2)))
    (is 0 (length (find-r-edge pool 'ra :from vertex3)))
    (ok (equalp (find-r-edge pool 'ra :from vertex1)
                (list edge2 edge1)))
    (ok (equalp (find-r-vertex pool 'ra :from vertex1)
                (list vertex3 vertex2)))
    (ok (equalp (find-r pool 'ra :from vertex1)
                (list (list :edge edge2 :vertex vertex3)
                      (list :edge edge1 :vertex vertex2))))
    ;; get-r
    (let ((ret (get-r pool 'ra :from vertex1 vertex2 edge-type)))
      (is edge1 (getf ret :edge))
      (is vertex2 (getf ret :vertex)))
    (let ((ret (get-r pool 'ra :from vertex1 vertex3 edge-type)))
      (is edge2 (getf ret :edge))
      (is vertex3 (getf ret :vertex)))
    (is edge1 (get-r-edge pool 'ra :from vertex1 vertex2 edge-type))
    (is edge2 (get-r-edge pool 'ra :from vertex1 vertex3 edge-type))
    (is vertex2 (get-r-vertex pool 'ra :from vertex1 vertex2 edge-type))
    (is vertex3 (get-r-vertex pool 'ra :from vertex1 vertex3 edge-type)))
  (up:snapshot pool))
...
(let ((vertex-note-1 "n-note-1")
  ...
  (let* ((vertex1 (tx-make-vertex pool 'vertex `((note ,vertex-note-1))))
         (vertex2 (tx-make-vertex pool 'vertex `((note ,vertex-note-2))))
         (edge1 (tx-make-edge pool 'ra vertex1 vertex2 edge-type-befor)))
    (is (id vertex1)   (from-id edge1))
    (is (id vertex2)   (to-id edge1))
