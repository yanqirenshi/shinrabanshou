...
(defmethod get-ghost ((pool banshou) (deccot deccot))
  "TODO: deccot -- :have --> ghost = (null) みたいな検索できんとイケんねぇ。
Graph理論をもっと勉強せんとイケん。
gremlin を参考にしよう。
それをS式にしてもエエんじゃろうけどね。"
...
;; make
(defgeneric make-deccot (banshou ghost &key name password note timestamp)
  (:documentation "デコットを作成します。
----------
TODO: 名称が tx-make-deccot の必要がある。
")
  (:method ((pool banshou) (creater ghost)
            &key
              (name "@未設定")
...
(defgeneric add-deccot (banshou place deccot type &rest slots)
  (:documentation "ghost に デコットを追加します。")
  (:method ((pool banshou) (ghost ghost) (deccot deccot) type &rest slots)
    ;; TODO: Check => ghost:deccot=1:n
    (when (get-ghost pool deccot)
      (error "このデコットはすでに誰かが所有しとるけぇ、アンタのもんにゃぁならんけぇ。"))
    ;; check type
    (when (not (eq type :own-a))
      (error "type は :own-a しか許しとらんのんよ。ごめんねぇ。type=~a" type))
    ;; do make
    (make-edge pool 'edge ghost deccot type slots)))
